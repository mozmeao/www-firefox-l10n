...
features-private-firefox-also-protects-your = { -brand-name-firefox } は、<a href="{ $url }">強化型トラッキング防止</a>でプライバシーを保護し、あらゆるサイトへ付きまとい、ユーザーの閲覧習慣に関する情報を収集するトラッカーをブロックします。有害なスクリプトやマルウェアからも保護します。

features-private-sidenote-we-are-not-big-tech = <em>補足:</em> Mozilla は大規模なテクノロジー企業ではありません。一般的な企業とは事業形態が異なります。株主がおらず独立していることで、利益よりも人を優先することができます。他の企業とは違い、ユーザーのデータを売り渡すことはしません。
...
features-private-read-firefoxs-privacy-notice = 詳細は <a href="{ $url }">{ -brand-name-firefox } のプライバシー通知</a>をぜひご覧ください。小さくないフォントですべてを網羅しています。
