...
features-adblocker-there-are-scores-of-content = Esistono moltissimi strumenti per il blocco dei contenuti, ma ecco alcune fantastiche opzioni a cura di { -brand-name-mozilla }…
# Variables:
#   $attrs (string) - link to https://addons.mozilla.org/firefox/addon/ublock-origin/ with other attributes
features-adblocker-considered-by-many-to-be-the = Considerato da molti il punto di riferimento per i blocchi pubblicitari, <a { $attrs }>uBlock Origin</a> è rinomato per la sua efficacia ed efficienza. Altamente personalizzabile, ma allo stesso tempo funziona perfettamente senza bisogno di mettere mano alla configurazione.
# Variables:
...
features-adblocker-adguard-adblocker-blunts = <a { $attrs1 }>AdGuard AdBlocker</a> riduce la pubblicità ovunque. Blocca le pubblicità sul Web e sui social media, e anche quei fastidiosi pop-up (se stai cercando un modo per bloccare i pop-up, il <a { $attrs2 }>Blocco pop-up</a> integrato è la scelta migliore).
# Variables:
...
features-adblocker-on-firefox-you-can-use = Su { -brand-name-firefox }, puoi usare le configurazioni di <a href="{ $privacy }">Privacy</a> o <a href="{ $blocking }">Blocco contenuti</a> per esercitare un controllo ancora più stretto sugli elementi traccianti che ti somministrano le pubblicità.
features-adblocker-choose-your-level-of-protection = Scegli il tuo livello di protezione
...
features-adblocker-do-it-yourself-custom = Protezione fai da te con il livello Personalizzato
features-adblocker-the-custom-setting-gives = La configurazione Personalizzato ti permette il massimo della scelta: puoi decidere tu cosa bloccare, compresi elementi traccianti, cookie e altro. Quando acconsenti all’utilizzo dei cookie su un sito web, passi automaticamente alla configurazione Personalizzato.
features-adblocker-cover-your-trail-block = Copri le tue tracce, blocca gli elementi traccianti
features-adblocker-click-on-the-trackers = Fai clic sulla casella Elementi traccianti e potrai scegliere due modi per bloccarli. Un’opzione per bloccare gli elementi traccianti è navigare in una Finestra anonima. L’altra opzione è bloccare gli elementi traccianti in tutte le finestre. Tieni però presente che, se scegli di bloccare tutti gli elementi traccianti, alcune pagine potrebbero non caricarsi correttamente.
features-adblocker-take-a-bite-out-of-cookies = Divora quei cookie
# Variables:
