...
compare-chrome-google-runs-the-worlds-largest = Google prevádzkuje najväčšiu reklamnú sieť na svete a Chrome je jej súčasťou.
# Variables:
# $howto (url) - link to https://support.mozilla.org/kb/switching-chrome-firefox
compare-chrome-switching-to-firefox-is-easy = Prechod na { -brand-name-firefox } je jednoduchý a rýchly – importujte svoje záložky prehliadača Chrome, svoje heslá, históriu a predvoľby jediným kliknutím a buďte okamžite pripravení používať { -brand-name-firefox }. Zistite, <a { $howto }>ako prejsť z prehliadača Chrome na { -brand-name-firefox }</a>.
