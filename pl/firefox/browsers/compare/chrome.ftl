...
compare-chrome-chrome-allows-other-websites-to = Chrome pozwala innym witrynom na zbieranie informacji o Tobie, aby mogły sugerować reklamy. Możesz zablokować określonym witrynom możliwość sugerowania reklam, ale nie możesz zatrzymać zbierania danych ani sugestii z innych witryn.
compare-chrome-google-runs-the-worlds-largest = Google posiada największą na świecie sieć reklamową, a Chrome jest jej częścią.
# Variables:
# $howto (url) - link to https://support.mozilla.org/kb/switching-chrome-firefox
compare-chrome-switching-to-firefox-is-easy = Przejście na { -brand-name-firefox(case: "gen") } jest łatwe i szybkie — zaimportuj zakładki, hasła, historię i preferencje z Chrome jednym kliknięciem i od razu bądź gotowy do użycia { -brand-name-firefox(case: "gen") }. Oto <a { $howto }>jak przejść z Chrome na  { -brand-name-firefox(case: "gen") }</a>.
