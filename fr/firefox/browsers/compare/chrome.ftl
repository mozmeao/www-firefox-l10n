...
compare-chrome-chrome-allows-other-websites-to = Chrome autorise également les autres sites web à collecter des informations à votre sujet pour vous proposer des publicités ciblées. Vous pouvez bloquer certains sites afin qu’ils ne vous suggèrent plus de publicités, mais vous ne pouvez pas empêcher la collecte de données ni les suggestions provenant d’autres sites.
compare-chrome-google-runs-the-worlds-largest = Google est à la tête du plus grand réseau publicitaire au monde, et Chrome en fait partie.

# Variables:
# $howto (url) - link to https://support.mozilla.org/kb/switching-chrome-firefox
compare-chrome-switching-to-firefox-is-easy = Passer à { -brand-name-firefox } est à la fois simple et rapide. Importez vos marque-pages, vos mots de passe, votre historique et vos préférences Chrome en un clic et retrouvez-les immédiatement dans { -brand-name-firefox }. Voici comment <a { $howto }>passer de Chrome à { -brand-name-firefox }.</a>
